...
export const PREGNANCY_CHECK_RESULTS: {
  value: PregnancyCheckResult;
  label: string;
  color: string;
}[] = [
 ...
 */
export function calculateExpectedDueDate(breedingDate: Date, species: AnimalSpecies): Date {
 ...
 */
export function calculatePregnancyProgress(breedingDate: Date, species: AnimalSpecies): number {
  const gestation = GESTATION_PERIODS[species];
...
export function getRecommendedCheckWindow(
  ...
  const recommendedMethod =
    (Object.entries(windows) as [PregnancyCheckMethod, CheckWindow][]).find(
      ([, window]) => window.recommended
    )?.[0] || 'ultrasound';
