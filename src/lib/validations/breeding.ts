...
export const breedingStatusSchema = z.enum(['pregnant', 'calved', 'failed', 'in_progress']);

export const createBreedingRecordSchema = z.object({
  animalId: idSchema,
  breedingDate: z.union([z.string().regex(/^\d{4}-\d{2}-\d{2}$/), z.date(), z.string().datetime()]),
  expectedCalvingDate: z
