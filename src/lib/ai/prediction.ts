...
export async function getMilkPrediction(input: MilkPredictionInput): Promise<MilkPredictionOutput> {
  ...
  try {
    ...
    console.error('Error generating milk prediction:', error);
...
function parsePredictionResponse(text: string, averageYield: number, currentTrend: 'increasing' | 'decreasing' | 'stable'): MilkPredictionOutput {
  try {
    ...
    return {
      ...
      recommendations: Array.isArray(parsed.recommendations) ? parsed.recommendations : ['Monitor production closely'],
      insights: Array.isArray(parsed.insights) ? parsed.insights : ['Production is within expected range'],
    };
  } catch (error) {
...
export async function getBatchMilkPredictions(animalIds: string[]): Promise<Map<string, MilkPredictionOutput>> {
  ...
  const promises = animalIds.map(async (id, index) => {
    ...
    try {
      ...
      console.error(`Failed to predict for animal ${id}:`, error);
