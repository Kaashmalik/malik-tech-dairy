...
import weatherService, { WeatherData } from '@/lib/weather/weather-service';
import { DEFAULT_PAKISTAN_CITIES } from '@/lib/weather/pakistan-cities';
        ...
        <div className='space-y-4'>
          ...
          <div className='grid grid-cols-2 gap-3 text-sm'>
            ...
            <div className='flex items-center space-x-2'>
              ...
              <span>
                Visibility: {weather.visibility ? Math.round(weather.visibility / 1000) : 'N/A'} km
              </span>
            </div>
            {weather.sunrise && (
              <div className='flex items-center space-x-2'>
...
export default function WeatherWidget() {
  ...
  const {
    ...
    queryFn: async () => {
      const response = await fetch(
        `/api/weather${selectedCity !== 'all' ? `?city=${selectedCity}` : ''}&limit=20`
      );
  ...
  const uniqueCities = weatherData
    ? [...new Set(weatherData.map((w: WeatherData) => w.city_name))]
    : [];
  ...
  return (
    <div className='space-y-4'>
            ...
            <Card>
              <CardContent className='pt-6'>
                <p className='text-muted-foreground text-center'>
                  No weather data available. Please add your OpenWeather API key to start fetching
                  weather data.
                </p>
              </CardContent>
            </Card>
      ...
      {!process.env.NEXT_PUBLIC_OPENWEATHER_API_KEY && (
        <Alert>
          <AlertDescription>
            To enable weather features, add your OpenWeather API key to your environment variables
            as NEXT_PUBLIC_OPENWEATHER_API_KEY. You can get a free API key from{' '}
            <a
