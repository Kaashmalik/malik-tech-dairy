...
import professionalWeatherService, {
  WeatherData,
} from '@/lib/weather/professional-weather-service';
        ...
        <div>
          ...
          {location && (
            <p className='text-muted-foreground mt-1 flex items-center gap-1 text-sm'>
              ...
              {location.city}, {location.country} • {location.latitude.toFixed(4)}°N,{' '}
              {location.longitude.toFixed(4)}°E
            </p>
          )}
        </div>
        <div className='flex gap-2'>
          <Button onClick={handleSyncWeather} variant='outline' size='sm' disabled={isRefreshing}>
      ...
      {isLoading ? (
        ...
        <div className='grid gap-4 md:grid-cols-1'>
          {weatherData.map((weather: WeatherData) => (
            <WeatherCard key={weather.id} weather={weather} location={location} />
          ))}
        </div>
      ) : (
        <Card>
