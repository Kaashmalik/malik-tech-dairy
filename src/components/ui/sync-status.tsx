...
export function SyncStatusIndicator({ tenantId, className }: SyncStatusIndicatorProps) {
  const { isOnline, pendingMutations, lastSync, syncInProgress, manualSync } =
    ...
    <div className='bg-card rounded-lg border p-6'>
      ...
      <div className='space-y-4'>
        ...
        <div className='flex items-center justify-between'>
          ...
          <span className='text-muted-foreground text-sm'>{formatLastSync(lastSync)}</span>
        </div>
        ...
        <div className='border-t pt-4'>
          ...
          {isOnline && pendingMutations === 0 && (
            <p className='text-muted-foreground mt-2 text-center text-xs'>All changes are synced</p>
          )}
        </div>
      </div>
    </div>
