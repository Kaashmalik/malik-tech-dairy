...
export function BreedingForm({ animalId, record, onClose, onSuccess }: BreedingFormProps) {
  ...
  const femaleAnimals =
    animals?.animals.filter(a => a.gender === 'female' && a.status === 'active') || [];
  const maleAnimals =
    animals?.animals.filter(a => a.gender === 'male' && a.status === 'active') || [];
  ...
  const mutation = useMutation({
    mutationFn: async (data: Partial<BreedingRecord>) => {
      const url = record ? `/api/breeding/${record.id}` : '/api/breeding';
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Record breeding information and track pregnancy</CardDescription>
      </CardHeader>
      <CardContent>
        <form onSubmit={handleSubmit(onSubmit)} className='space-y-4'>
          {!animalId && (
            <div>
              ...
              {errors.animalId && <p className='mt-1 text-sm text-red-500'>Animal is required</p>}
            </div>
          )}
          ...
          <div>
            ...
            />
            {errors.breedingDate && (
              <p className='mt-1 text-sm text-red-500'>Breeding date is required</p>
            )}
          </div>
          ...
          <div>
            ...
            <Input id='expectedCalvingDate' type='date' {...register('expectedCalvingDate')} />
          ...
          {record && (
            <>
              <div>
                ...
                <Input id='actualCalvingDate' type='date' {...register('actualCalvingDate')} />
              </div>
