...
export function PregnancyCheckForm({
  ...
  return (
    <Card className='border-0 shadow-lg'>
      <CardHeader className='rounded-t-lg bg-gradient-to-r from-blue-500 to-indigo-500 text-white'>
      ...
      <CardContent className='pt-6'>
        <form onSubmit={handleSubmit(onSubmit)} className='space-y-5'>
          ...
          <div className='space-y-2'>
            ...
            />
            {errors.checkDate && <p className='text-sm text-red-500'>Check date is required</p>}
          </div>
              ...
              {PREGNANCY_CHECK_RESULTS.map(resultOption => (
                <button
                  ...
                  className={`flex flex-col items-center gap-2 rounded-lg border-2 p-4 transition-all ${
                    selectedResult === resultOption.value
                ...
                >
                  ...
                  <span className='text-sm font-medium'>{resultOption.label}</span>
                </button>
              ))}
            </div>
          </div>
