    ...
    <div className='space-y-6'>
      ...
      <div className='space-y-2'>
        <div className='flex items-start justify-between'>
          <div>
            ...
            {(disease.nameUrdu || disease.localName) && (
              <p className='text-lg text-gray-600'>{disease.nameUrdu || disease.localName}</p>
            )}
          </div>
        ...
        <div className='flex flex-wrap gap-2'>
          ...
          {disease.isNotifiable && <Badge variant='destructive'>Notifiable Disease</Badge>}
        </div>
      </div>
                  ...
                  <div className='mt-1 flex items-center gap-1'>
                    {impactStars.map((_, i) => (
                      <span
                        key={i}
                        className={
                          i < disease.economicImpactScore ? 'text-yellow-500' : 'text-gray-300'
                        }
                      >
                        ★
                      </span>
          ...
          {disease.zoonotic && (
            <Alert className='border-red-200 bg-red-50'>
              ...
              <AlertDescription className='text-red-700'>
                This disease can be transmitted from animals to humans. Take appropriate precautions
            ...
            {disease.treatments && disease.treatments.length > 0 && (
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Medicines commonly used for treatment</CardDescription>
                </CardHeader>
                <CardContent>
            ...
            {disease.protocols && disease.protocols.length > 0 && (
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Standard treatment guidelines</CardDescription>
                </CardHeader>
                <CardContent>
                  <div className='space-y-4'>
                    {disease.protocols.map((protocol, index) => (
                      <div key={index} className='rounded-lg border p-4'>
                        ...
                        <p className='mb-2 text-sm text-gray-600'>{protocol.severity_level}</p>
                        <p className='text-sm text-gray-500'>
                          Success rate: {protocol.success_rate} | Recovery:{' '}
                          {protocol.expected_recovery_days} days
                        </p>
                      </div>
                    ))}
                  </div>
                    ...
                    {disease.vaccination_schedules.map((schedule, index) => (
                      <div key={index} className='rounded-lg border p-3'>
                        <div className='mb-1 flex items-center justify-between'>
                          ...
                          <Badge
                            variant={schedule.priority === 'essential' ? 'default' : 'secondary'}
                          >
                            {schedule.priority}
                          </Badge>
                        </div>
                        <p className='text-sm text-gray-600'>
                          Age: {schedule.animal_age_start_label} | Route: {schedule.route} | Dose:{' '}
                          {schedule.dosage}
                        </p>
          ...
          {disease.isNotifiable && (
            <Alert className='border-amber-200 bg-amber-50'>
              ...
              <AlertDescription className='text-amber-700'>
                This disease must be reported to the veterinary authorities. Contact your local
                veterinary department immediately upon suspicion.
              </AlertDescription>
            </Alert>
          )}
        </TabsContent>
