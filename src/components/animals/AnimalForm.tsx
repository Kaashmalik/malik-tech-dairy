...
export function AnimalForm({ animalId, initialData, onSuccess }: AnimalFormProps) {
  ...
  const { canAddAnimal, animalCount, limits } = useTenantLimits();
  ...
  const createAnimal = useOfflineCreateAnimal(
    (initialData?.customFields?.tenantId as string) || 'default-tenant'
  ); // TODO: Get actual tenantId
  const updateAnimal = useOfflineUpdateAnimal(
    (initialData?.customFields?.tenantId as string) || 'default-tenant'
  );
      ...
      <Card className='overflow-hidden border-0 shadow-xl'>
        ...
        <div className={`bg-gradient-to-r ${speciesInfo.color} p-6 text-white`}>
          <div className='flex items-center gap-4'>
            ...
            <div>
              <h1 className='text-2xl font-bold'>{animalId ? 'Edit Animal' : 'Add New Animal'}</h1>
          ...
          {!animalId && (
            <div className='mt-4 flex items-center gap-2 rounded-xl bg-white/10 px-4 py-2 backdrop-blur'>
              ...
              <span className='text-sm'>
                {limits?.maxAnimals === -1
                  ? `${animalCount} animals registered (Unlimited)`
                  : `${animalCount} of ${limits?.maxAnimals || 0} animals used`}
              </span>
              {!canAddAnimal && (
          ...
          <form onSubmit={handleSubmit(onSubmit)} className='space-y-6'>
            ...
            <div className='grid gap-4 md:grid-cols-2'>
              ...
              <div>
                ...
                <Input {...register('name')} placeholder='e.g., Bella' className='h-11' />
              </div>
            </div>
            {/* Species Selection */}
            <div>
            ...
            <div className='grid gap-4 md:grid-cols-2'>
              ...
              <div>
                ...
                <Input type='date' {...register('purchaseDate')} className='h-11' />
              </div>
            </div>
            {/* Purchase Price */}
            <div>
