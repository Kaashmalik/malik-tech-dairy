...
import { parseCSV, parseExcel, validateImportFile, AnimalImportData } from "@/lib/import/csv-parser";
...
async function importAnimals(data: AnimalImportData[], tenantId: string) {
  ...
  try {
    ...
    if (availableSlots <= 0) {
      results.errors.push(`You've reached your limit of ${maxAnimals} animals. Please upgrade your subscription to add more animals.`);
      return results;
    }

    if (data.length > availableSlots) {
      results.errors.push(`You can only import ${availableSlots} more animals with your current subscription. ${data.length} animals were in the file.`);
  ...
  for (const animal of data) {
    try {
      ...
      results.errors.push(`Error importing ${animal.tag}: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
...
async function importMilkRecords(data: any[], tenantId: string) {
  ...
  try {
    ...
    if (availableSlots <= 0) {
      results.errors.push(`You've reached your limit of ${maxMilkRecords} milk records. Please upgrade your subscription to add more records.`);
      return results;
    }

    if (data.length > availableSlots) {
      results.errors.push(`You can only import ${availableSlots} more milk records with your current subscription. ${data.length} records were in the file.`);
      // Limit the import to available slots
      data = data.slice(0, availableSlots);
    }
  } catch (error) {
  ...
  for (const record of data) {
    try {
      ...
      if (existing) {
        results.skipped++;
        results.errors.push(`Milk record for ${validated.animalTag} on ${validated.date} already exists`);
        continue;
      }
      ...
      if (error) {
        results.errors.push(`Failed to import milk record for ${validated.animalTag}: ${error.message}`);
      } else {
        results.imported++;
      }
    } catch (error) {
      results.errors.push(`Error importing milk record: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
...
async function importHealthRecords(data: any[], tenantId: string) {
  ...
  try {
    ...
    if (availableSlots <= 0) {
      results.errors.push(`You've reached your limit of ${maxHealthRecords} health records. Please upgrade your subscription to add more records.`);
      return results;
    }

    if (data.length > availableSlots) {
      results.errors.push(`You can only import ${availableSlots} more health records with your current subscription. ${data.length} records were in the file.`);
      // Limit the import to available slots
      data = data.slice(0, availableSlots);
    }
  } catch (error) {
  ...
  for (const record of data) {
    try {
      ...
      if (error) {
        results.errors.push(`Failed to import health record for ${validated.animalTag}: ${error.message}`);
      } else {
        results.imported++;
      }
    } catch (error) {
      results.errors.push(`Error importing health record: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
...
function generateAnimalTemplate(): string {
  ...
  const sampleData = [
    ...
    ['', '', 'cow/buffalo/goat/sheep', '', 'male/female', 'YYYY-MM-DD', '', '', '', 'active/sold/deceased', ''],
  ];
...
function generateHealthTemplate(): string {
  const headers = ['AnimalTag', 'Date', 'Type', 'Description', 'Treatment', 'Veterinarian', 'Notes'];
  const sampleData = [
    ['TAG001', '2024-01-01', 'Vaccination', 'Annual vaccination', 'Standard vaccine', 'Dr. Smith', 'No adverse reactions'],
    ['', 'YYYY-MM-DD', 'Checkup/Treatment/Vaccination', 'Description', 'Treatment given', 'Vet name', 'Optional notes'],
  ];
