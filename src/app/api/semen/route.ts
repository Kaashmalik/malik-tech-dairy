  ...
  return withTenantContext(async (req, context) => {
    try {
      ...
      if (error) {
        ...
        return NextResponse.json(createApiError('Failed to fetch semen inventory', 'FETCH_ERROR'), {
          status: 500,
        });
      }
      ...
      // Summary
      const summary = {
        totalStraws: transformedInventory.reduce(
          (sum, i) => sum + (i.status === 'available' ? i.quantity : 0),
          0
        ),
        totalItems: transformedInventory.length,
        available: transformedInventory.filter(i => i.status === 'available' && i.quantity > 0)
          .length,
...
export async function POST(request: NextRequest) {
  return withTenantContext(async (req, context) => {
    try {
      ...
      if (error) {
        logger.error('Database error creating semen inventory', error, {
  ...
  return withTenantContext(async (req, context) => {
    try {
      ...
      if (updateData.storageLocation !== undefined)
        updateObj.storage_location = updateData.storageLocation;
      ...
      if (error) {
        logger.error('Database error updating semen inventory', error, {
          tenantId: context.tenantId,
        });
        return NextResponse.json(
...
export async function DELETE(request: NextRequest) {
  return withTenantContext(async (req, context) => {
    try {
      ...
      if (error) {
        logger.error('Database error deleting semen inventory', error, {
          tenantId: context.tenantId,
        });
        return NextResponse.json(
          {
            success: false,
            error: 'Failed to delete semen inventory item',
