...
export async function GET(request: NextRequest) {
  return withTenantContext(async (req, context) => {
    try {
      ...
      const { data: pregnantRecords, error: breedingError } = (await breedingQuery) as {
        data: any[] | null;
        error: any;
      };
      ...
      if (breedingError) {
        ...
        return NextResponse.json(
          createApiError('Failed to fetch pregnant animals', 'FETCH_ERROR'),
          { status: 500 }
        );
      }
      ...
      const pregnantAnimals = (pregnantRecords || []).map(record => {
        ...
        return {
          ...
          daysRemainingFormatted:
            daysRemaining !== null ? formatDaysRemaining(daysRemaining) : 'Unknown',
      ...
      const dueThisWeek = pregnantAnimals.filter(
        a => !a.isOverdue && a.daysRemaining !== null && a.daysRemaining <= 7
      );
      const dueThisMonth = pregnantAnimals.filter(
        a =>
      ...
      const summary = {
        ...
        awaitingConfirmation: pregnantAnimals.filter(a => !a.breedingRecord.pregnancyConfirmed)
          .length,
