    ...
    try {
      ...
      if (error) {
        ...
        return NextResponse.json(
          createApiError('Failed to fetch pregnancy checks', 'FETCH_ERROR'),
          { status: 500 }
        );
      }
...
export async function POST(request: NextRequest) {
  return withTenantContext(async (req, context) => {
    try {
      ...
      const { breedingRecordId, animalId, checkDate, checkMethod, result, vetName, notes, cost } =
        body;
        ...
        return NextResponse.json(
          {
            success: false,
            error:
              'Missing required fields: breedingRecordId, animalId, checkDate, checkMethod, result',
          },
          { status: 400 }
        );
      }
      ...
      if (!validMethods.includes(checkMethod)) {
        return NextResponse.json(
          {
            success: false,
            error: `Invalid checkMethod. Must be one of: ${validMethods.join(', ')}`,
          },
          { status: 400 }
        );
      }
