...
export const farmApplicationStatusEnum = pgEnum('farm_application_status', [
  'pending',
  'payment_uploaded',
  'under_review',
  'approved',
...
export const diseaseSeverityEnum = pgEnum('disease_severity', [
  'mild',
  'moderate',
  'severe',
  'critical',
]);
export const treatmentOutcomeEnum = pgEnum('treatment_outcome', [
  'pending',
  'recovering',
  'recovered',
  'chronic',
...
export const customFieldsConfig = pgTable(
...
export const farmApplications = pgTable(
  'farm_applications',
  {
    id: text('id').primaryKey(),
  ...
  table => ({
    ...
    assignedFarmIdIdx: uniqueIndex('farm_applications_assigned_farm_id_idx').on(
      table.assignedFarmId
    ),
...
export const farmIdSequence = pgTable(
  'farm_id_sequence',
  {
...
export const diseases = pgTable(
  'diseases',
  {
    ...
    treatmentProtocols: jsonb('treatment_protocols')
      .$type<
        Array<{
          medication: string;
          dosage: string;
    ...
    commonInAnimalTypes: jsonb('common_in_animal_types')
      .$type<string[]>()
      .default(['cow', 'buffalo']),
    seasonalPrevalence: jsonb('seasonal_prevalence')
      .$type<
        Array<{
          season: string;
          prevalence: string;
...
export const treatmentRecords = pgTable(
  'treatment_records',
  {
    ...
    diseaseId: text('disease_id')
      .notNull()
      .references(() => diseases.id, { onDelete: 'restrict' }),
    symptomsObserved: jsonb('symptoms_observed').$type<string[]>().notNull(),
    diagnosis: text('diagnosis').notNull(),
    treatmentGiven: jsonb('treatment_given')
      .$type<
        Array<{
          medication: string;
          dosage: string;
...
export const veterinaryProtocols = pgTable(
  'veterinary_protocols',
  {
    id: text('id').primaryKey(),
    diseaseId: text('disease_id')
      .notNull()
...
export const feedingSchedules = pgTable(
  'feeding_schedules',
  {
    id: text('id').primaryKey(),
    tenantId: text('tenant_id')
...
export const nutritionTemplates = pgTable(
  'nutrition_templates',
  {
    id: text('id').primaryKey(),
    tenantId: text('tenant_id')
    ...
    dailyFeedRequirements: jsonb('daily_feed_requirements')
      .$type<
        Array<{
          feedType: string;
          quantity: number;
...
export const milkQualityTests = pgTable(
  'milk_quality_tests',
  {
    id: text('id').primaryKey(),
    tenantId: text('tenant_id')
...
// ============================================================================
// NOTE: Enhanced tables (genetic_profiles, feed_inventory, etc.) exist in Supabase
// but are accessed via REST API, not Drizzle ORM. See MEMORY.md for details.

// Export enhanced types only (no circular dependency)
export type { EnhancedAnimalProfile, AnimalSearchFilters, BatchOperation } from './schema-enhanced';
