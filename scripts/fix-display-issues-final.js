...
async function fixDisplayIssues() {
  try {
    ...
    if (tenantError) {
      ...
      await supabase.from('weather_data').delete().eq('tenant_id', tenantId);
    ...
    console.log(
      `   ✅ OpenWeather API: ${process.env.NEXT_PUBLIC_OPENWEATHER_API_KEY ? 'Set' : 'NOT SET'}`
    );
    ...
    finalWeather?.forEach(w =>
      console.log(`     - ${w.city_name}: ${w.temperature}°C, ${w.weather_description}`)
    );
