# MTK Dairy - Complete System Flow Documentation

> **Enterprise-Level Multi-Tenant Dairy Farm SaaS Platform**
>
> Version: 1.0.0 | Last Updated: December 2024
>
> Real-World Example: `kaash0542@gmail.com` → `Green Valley Farm` → `FARM-N669KHLF`

---

## Table of Contents

1. [Architecture Overview](#1-architecture-overview)
2. [Technology Stack](#2-technology-stack)
3. [Database Schema](#3-database-schema)
4. [User Roles & Permissions](#4-user-roles--permissions)
5. [Complete User Journey Flow](#5-complete-user-journey-flow)
6. [API Routes Reference](#6-api-routes-reference)
7. [Real-World Example Walkthrough](#7-real-world-example-walkthrough)
8. [Security & Best Practices](#8-security--best-practices)
9. [AI Agent Rules & Memory](#9-ai-agent-rules--memory)

---

## 1. Architecture Overview

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                           MTK DAIRY ARCHITECTURE                             │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  ┌─────────────┐     ┌─────────────┐     ┌─────────────┐                   │
│  │   Browser   │────▶│   Next.js   │────▶│   Clerk     │                   │
│  │   Client    │     │   15 App    │     │   Auth      │                   │
│  └─────────────┘     └──────┬──────┘     └─────────────┘                   │
│                             │                                               │
│                             ▼                                               │
│  ┌──────────────────────────────────────────────────────────────────────┐  │
│  │                          API LAYER                                    │  │
│  │  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐             │  │
│  │  │ /api/    │  │ /api/    │  │ /api/    │  │ /api/    │             │  │
│  │  │ farm-    │  │ admin/   │  │ animals/ │  │ user/    │             │  │
│  │  │ apps     │  │          │  │ milk/    │  │ farms    │             │  │
│  │  └──────────┘  └──────────┘  └──────────┘  └──────────┘             │  │
│  └──────────────────────────────────────────────────────────────────────┘  │
│                             │                                               │
│                             ▼                                               │
│  ┌──────────────────────────────────────────────────────────────────────┐  │
│  │                        DATA LAYER                                     │  │
│  │  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐                │  │
│  │  │   Supabase   │  │   Clerk      │  │  Cloudinary  │                │  │
│  │  │   PostgreSQL │  │   Orgs       │  │   Storage    │                │  │
│  │  └──────────────┘  └──────────────┘  └──────────────┘                │  │
│  └──────────────────────────────────────────────────────────────────────┘  │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```

### Multi-Tenancy Model

- **Clerk Organizations** = Farm Tenants
- **Organization ID** = Tenant ID in Supabase
- **User can belong to multiple organizations** (farms)

---

## 2. Technology Stack

| Layer          | Technology               | Purpose                        |
| -------------- | ------------------------ | ------------------------------ |
| **Frontend**   | Next.js 15 (App Router)  | React framework with SSR       |
| **UI**         | Tailwind CSS + shadcn/ui | Styling & components           |
| **Auth**       | Clerk                    | Authentication & Organizations |
| **Primary DB** | Supabase PostgreSQL      | Relational data storage        |
| **ORM**        | Drizzle ORM              | Type-safe database queries     |
| **Storage**    | Cloudinary               | Images & payment slips         |
| **State**      | TanStack Query + Zustand | Client state management        |
| **Email**      | Resend                   | Transactional emails           |
| **i18n**       | next-intl                | English + Urdu (RTL)           |

---

## 3. Database Schema

### Core Tables

```sql
-- Tenants (Farms)
tenants
├── id (PK) ─────────────────── Clerk Organization ID (org_xxx)
├── slug ────────────────────── URL-friendly name (green-valley)
├── farm_name ───────────────── Display name (Green Valley Farm)
├── animal_types ────────────── JSON array ['cow', 'buffalo']
├── language ────────────────── 'en' | 'ur'
├── currency ────────────────── 'PKR'
└── timezone ────────────────── 'Asia/Karachi'

-- Platform Users
platform_users
├── id (PK) ─────────────────── Clerk User ID (user_xxx)
├── email ───────────────────── User email
├── platform_role ───────────── 'super_admin' | 'admin' | 'user'
└── is_active ───────────────── Account status

-- Farm Applications
farm_applications
├── id (PK) ─────────────────── Application ID
├── applicant_id (FK) ───────── References platform_users
├── farm_name ───────────────── Requested farm name
├── status ──────────────────── 'pending' | 'payment_uploaded' | 'approved' | 'rejected'
├── requested_plan ──────────── 'free' | 'professional' | 'farm' | 'enterprise'
├── payment_slip_url ────────── Cloudinary URL
├── assigned_farm_id ────────── Generated ID (FARM-N669KHLF)
└── assigned_tenant_id ──────── References tenants (after approval)

-- Tenant Members
tenant_members
├── id (PK)
├── tenant_id (FK) ──────────── References tenants
├── user_id (FK) ────────────── References platform_users
├── role ────────────────────── 'farm_owner' | 'farm_manager' | 'staff'
└── status ──────────────────── 'active' | 'inactive'

-- Subscriptions
subscriptions
├── id (PK)
├── tenant_id (FK) ──────────── References tenants
├── plan ────────────────────── Subscription plan
├── status ──────────────────── 'active' | 'trial' | 'expired'
└── renew_date ──────────────── Next payment date
```

---

## 4. User Roles & Permissions

### Platform Roles

| Role          | Description            | Access                                  |
| ------------- | ---------------------- | --------------------------------------- |
| `super_admin` | Platform administrator | Full access to all farms & applications |
| `admin`       | Limited admin          | Specific admin functions                |
| `user`        | Regular user           | Own farms only                          |

### Tenant (Farm) Roles

| Role            | Description | Permissions                    |
| --------------- | ----------- | ------------------------------ |
| `farm_owner`    | Farm owner  | Full access to farm            |
| `farm_manager`  | Manager     | Most operations except billing |
| `veterinarian`  | Vet         | Health records, breeding       |
| `milking_staff` | Staff       | Milk logs only                 |
| `accountant`    | Finance     | Expenses, sales, reports       |
| `guest`         | Read-only   | View only                      |

---

## 5. Complete User Journey Flow

### Phase 1: Authentication

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                         AUTHENTICATION FLOW                                  │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  User visits maliktechdairy.com                                             │
│         │                                                                   │
│         ▼                                                                   │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │  Is User Authenticated?                                              │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│         │                                                                   │
│    NO   │   YES                                                             │
│         │    │                                                              │
│         ▼    │                                                              │
│  /sign-up    │     ┌───────────────────────────────────────────────┐       │
│  or          │     │  Has Active Organization (Farm)?              │       │
│  /sign-in    │     └───────────────────────────────────────────────┘       │
│         │          │                                                        │
│         │     NO   │   YES                                                  │
│         │          │    │                                                   │
│         ▼          ▼    ▼                                                   │
│  Clerk Auth   /select-farm   /dashboard                                     │
│         │          │              │                                         │
│         ▼          ▼              ▼                                         │
│  Create platform_users record   Show farms or   Farm Dashboard              │
│         │                       redirect to                                 │
│         ▼                       /apply                                      │
│  Check if has farms                                                         │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```

### Phase 2: Farm Application

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                      FARM APPLICATION FLOW                                   │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  User visits /apply                                                         │
│         │                                                                   │
│         ▼                                                                   │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │  GET /api/user/farms - Check existing applications                   │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│         │                                                                   │
│         ├── Has approved farm? ───────────▶ Redirect to /select-farm       │
│         │                                                                   │
│         ├── Has pending application? ─────▶ Show Application Status        │
│         │                                    ├── Farm Name                  │
│         │                                    ├── Status Badge               │
│         │                                    └── "Wait for approval"        │
│         │                                                                   │
│         └── No application? ──────────────▶ Show Application Form          │
│                                                                             │
│  APPLICATION FORM (Multi-Step)                                              │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │  Step 1: Farm Details                                                │   │
│  │  ├── Farm Name *                                                     │   │
│  │  ├── Owner Name * (auto-filled from Clerk)                           │   │
│  │  ├── Email * (auto-filled from Clerk)                                │   │
│  │  ├── Phone *                                                         │   │
│  │  ├── Address                                                         │   │
│  │  ├── City                                                            │   │
│  │  └── Province (dropdown)                                             │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │  Step 2: Animal Details                                              │   │
│  │  ├── Animal Types * (multi-select: cow, buffalo, goat, etc.)         │   │
│  │  └── Estimated Animals *                                             │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │  Step 3: Select Plan                                                 │   │
│  │  ├── Free (0 PKR) - 5 animals, 1 user                                │   │
│  │  ├── Professional (4,999 PKR/mo) - 100 animals, 5 users              │   │
│  │  ├── Farm (12,999 PKR/mo) - 500 animals, 15 users                    │   │
│  │  └── Enterprise (Custom) - Unlimited                                 │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │  Step 4: Payment (Only for Paid Plans)                               │   │
│  │  ├── Bank Transfer Details (Meezan Bank)                             │   │
│  │  ├── JazzCash Details                                                │   │
│  │  └── Upload Payment Slip (Cloudinary)                                │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                                                             │
│  On Submit:                                                                 │
│  POST /api/farm-applications                                                │
│  ├── Create farm_applications record                                       │
│  ├── Status = 'pending' (free) or 'payment_uploaded' (paid)                │
│  └── Show success message with application status                          │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```

### Phase 3: Super Admin Review

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                       SUPER ADMIN REVIEW FLOW                                │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  Super Admin visits /super-admin/applications                               │
│         │                                                                   │
│         ▼                                                                   │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │  GET /api/admin/applications                                         │   │
│  │  └── Returns all applications with filters                          │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│         │                                                                   │
│         ▼                                                                   │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │  Applications Dashboard                                              │   │
│  │  ├── Search: Farm name, owner, email, phone                          │   │
│  │  ├── Filter: All, Pending, Payment Uploaded, Approved, Rejected      │   │
│  │  ├── Results count: "Showing X of Y applications"                    │   │
│  │  │                                                                   │   │
│  │  │  MOBILE VIEW (Cards):                                             │   │
│  │  │  ┌───────────────────────────────────────────────────────────┐   │   │
│  │  │  │ Green Valley Farm           [● Payment Uploaded]          │   │   │
│  │  │  │ Malik Kashif                                              │   │   │
│  │  │  │ FARM-N669KHLF                                             │   │   │
│  │  │  │ Email: kaash0542@gmail.com  Phone: 03038111297            │   │   │
│  │  │  │ Plan: Free                  Date: Dec 4, 2025             │   │   │
│  │  │  │ [View Payment Slip]                                       │   │   │
│  │  │  │ [✓ Approve]  [✗ Reject]                                   │   │   │
│  │  │  └───────────────────────────────────────────────────────────┘   │   │
│  │  │                                                                   │   │
│  │  │  DESKTOP VIEW (Table):                                            │   │
│  │  │  | Farm | Owner | Contact | Plan | Status | Payment | Actions |   │   │
│  │  │                                                                   │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                                                             │
│  APPROVAL ACTION:                                                           │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │  POST /api/admin/applications/{id}/review                            │   │
│  │  Body: { action: 'approve', reviewNotes: '...' }                     │   │
│  │                                                                      │   │
│  │  APPROVAL PROCESS:                                                   │   │
│  │  1. Generate Farm ID: FARM-XXXXXXXX                                  │   │
│  │  2. Create Clerk Organization (with retry logic)                     │   │
│  │     └── Try slug variations if taken                                 │   │
│  │  3. Add applicant as org:admin member                                │   │
│  │  4. Create tenant record in Supabase                                 │   │
│  │  5. Create subscription record                                       │   │
│  │  6. Create tenant_members record                                     │   │
│  │  7. Update farm_applications status = 'approved'                     │   │
│  │  8. Send approval email (Resend)                                     │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                                                             │
│  REJECTION ACTION:                                                          │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │  POST /api/admin/applications/{id}/review                            │   │
│  │  Body: { action: 'reject', rejectionReason: '...' }                  │   │
│  │                                                                      │   │
│  │  REJECTION PROCESS:                                                  │   │
│  │  1. Update farm_applications status = 'rejected'                     │   │
│  │  2. Set rejection_reason                                             │   │
│  │  3. Send rejection email (Resend)                                    │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```

### Phase 4: User Access After Approval

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                     POST-APPROVAL USER FLOW                                  │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  User visits /apply (after approval)                                        │
│         │                                                                   │
│         ▼                                                                   │
│  GET /api/user/farms                                                        │
│         │                                                                   │
│         ├── Returns: { hasFarms: true, farms: [...], applications: [...] } │
│         │                                                                   │
│         ▼                                                                   │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │  Application Status Page                                             │   │
│  │  ┌───────────────────────────────────────────────────────────────┐  │   │
│  │  │         ✓ Welcome to MTK Dairy!                               │  │   │
│  │  │                                                               │  │   │
│  │  │  Congratulations! Your farm has been approved.                │  │   │
│  │  │                                                               │  │   │
│  │  │  ┌─────────────────────────────────────────────────────────┐  │  │   │
│  │  │  │ Farm Name    │  Green Valley Farm                       │  │  │   │
│  │  │  │ Status       │  ✓ Approved                              │  │  │   │
│  │  │  │ Farm ID      │  FARM-N669KHLF                           │  │  │   │
│  │  │  └─────────────────────────────────────────────────────────┘  │  │   │
│  │  │                                                               │  │   │
│  │  │  [          Go to Dashboard  →          ]                     │  │   │
│  │  │                                                               │  │   │
│  │  └───────────────────────────────────────────────────────────────┘  │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│         │                                                                   │
│         ▼                                                                   │
│  User clicks "Go to Dashboard"                                              │
│         │                                                                   │
│         ▼                                                                   │
│  /select-farm                                                               │
│         │                                                                   │
│         ▼                                                                   │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │  Check Clerk Organization Memberships                                │   │
│  │  useOrganizationList() from @clerk/nextjs                            │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│         │                                                                   │
│         ├── Has Clerk Org membership? ────▶ Show farm list                 │
│         │   User clicks farm → setActive() → /dashboard                    │
│         │                                                                   │
│         └── No Clerk Org membership? ─────▶ Show "Join Farm" button        │
│             │                                                               │
│             ▼                                                               │
│  POST /api/user/join-org                                                    │
│  ├── Find approved application                                             │
│  ├── Find tenant by farm_name                                              │
│  ├── If tenant.id starts with 'org_':                                      │
│  │   └── Add user to Clerk organization                                    │
│  ├── If tenant.id is fallback (tenant_xxx):                                │
│  │   ├── Create new Clerk organization                                     │
│  │   ├── Update tenant.id to new org_xxx                                   │
│  │   ├── Update subscriptions.tenant_id                                    │
│  │   ├── Update tenant_members.tenant_id                                   │
│  │   └── Add user to Clerk organization                                    │
│  └── Refresh page → User sees farm → Clicks → Dashboard                    │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```

### Phase 5: Farm Dashboard

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                        FARM DASHBOARD FLOW                                   │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  /dashboard (requires orgId in Clerk session)                               │
│         │                                                                   │
│         ▼                                                                   │
│  Dashboard Layout checks:                                                   │
│  ├── userId? → No → /sign-in                                                │
│  └── orgId? → No → /select-farm                                             │
│         │                                                                   │
│         ▼                                                                   │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │  Farm Dashboard                                                      │   │
│  │  ┌───────────────────────────────────────────────────────────────┐  │   │
│  │  │  📊 Quick Stats                                               │  │   │
│  │  │  ├── Total Animals: 45                                        │  │   │
│  │  │  ├── Today's Milk: 320 liters                                 │  │   │
│  │  │  ├── Eggs Collected: 150                                      │  │   │
│  │  │  └── Health Alerts: 2                                         │  │   │
│  │  └───────────────────────────────────────────────────────────────┘  │   │
│  │                                                                      │   │
│  │  Navigation:                                                         │   │
│  │  ├── /dashboard/animals   - Animal management                        │   │
│  │  ├── /dashboard/milk      - Milk logging                             │   │
│  │  ├── /dashboard/health    - Health records                           │   │
│  │  ├── /dashboard/breeding  - Breeding management                      │   │
│  │  ├── /dashboard/finance   - Expenses & sales                         │   │
│  │  ├── /dashboard/analytics - Reports & insights                       │   │
│  │  ├── /dashboard/staff     - Staff management                         │   │
│  │  └── /dashboard/settings  - Farm settings                            │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                                                             │
│  API ROUTES (all require orgId header):                                     │
│  ├── GET/POST /api/animals                                                  │
│  ├── GET/POST /api/milk                                                     │
│  ├── GET/POST /api/health/records                                           │
│  ├── GET/POST /api/breeding                                                 │
│  ├── GET/POST /api/expenses                                                 │
│  ├── GET/POST /api/sales                                                    │
│  └── GET /api/analytics                                                     │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```

---

## 6. API Routes Reference

### Authentication & User Routes

| Method | Route                | Description                              |
| ------ | -------------------- | ---------------------------------------- |
| GET    | `/api/user/farms`    | Get user's farms and applications        |
| POST   | `/api/user/join-org` | Join user to their approved organization |

### Farm Application Routes

| Method | Route                         | Description              |
| ------ | ----------------------------- | ------------------------ |
| POST   | `/api/farm-applications`      | Submit new application   |
| GET    | `/api/farm-applications`      | List user's applications |
| GET    | `/api/farm-applications/{id}` | Get application details  |
| PATCH  | `/api/farm-applications/{id}` | Upload payment slip      |

### Super Admin Routes

| Method | Route                                 | Description                |
| ------ | ------------------------------------- | -------------------------- |
| GET    | `/api/admin/applications`             | List all applications      |
| POST   | `/api/admin/applications/{id}/review` | Approve/reject application |
| GET    | `/api/admin/stats`                    | Dashboard statistics       |
| GET    | `/api/admin/tenants`                  | List all tenants           |
| GET    | `/api/admin/users`                    | List all users             |

### Tenant (Farm) Routes

| Method   | Route                 | Description      |
| -------- | --------------------- | ---------------- |
| GET/POST | `/api/animals`        | Animal CRUD      |
| GET/POST | `/api/milk`           | Milk log CRUD    |
| GET/POST | `/api/health/records` | Health records   |
| GET/POST | `/api/breeding`       | Breeding records |
| GET/POST | `/api/expenses`       | Expense tracking |
| GET/POST | `/api/sales`          | Sales tracking   |
| GET      | `/api/analytics`      | Farm analytics   |

---

## 7. Real-World Example Walkthrough

### User: kaash0542@gmail.com → Green Valley Farm

#### Step 1: Sign Up (December 4, 2025)

```javascript
// User signs up via Clerk
{
  userId: "user_2qXXXXXXXXXX",
  email: "kaash0542@gmail.com",
  name: "Malik Kashif"
}

// Clerk webhook creates platform_users record
INSERT INTO platform_users (id, email, name, platform_role)
VALUES ('user_2qXXXX', 'kaash0542@gmail.com', 'Malik Kashif', 'user');
```

#### Step 2: Apply for Farm (December 4, 2025)

```javascript
// POST /api/farm-applications
{
  farmName: "Green Valley Farm",
  ownerName: "Malik Kashif",
  email: "kaash0542@gmail.com",
  phone: "03038111297",
  city: "Lahore",
  province: "Punjab",
  animalTypes: ["cow", "buffalo"],
  estimatedAnimals: 25,
  requestedPlan: "free"
}

// Creates farm_applications record
INSERT INTO farm_applications (
  id, applicant_id, farm_name, status, requested_plan
) VALUES (
  'app_xxx', 'user_2qXXXX', 'Green Valley Farm', 'pending', 'free'
);
```

#### Step 3: Super Admin Approval (December 4, 2025)

```javascript
// POST /api/admin/applications/app_xxx/review
{
  action: "approve",
  reviewNotes: "Verified application"
}

// APPROVAL PROCESS:
// 1. Generate Farm ID
farmId = "FARM-N669KHLF"

// 2. Create Clerk Organization
clerkOrg = await clerk.organizations.createOrganization({
  name: "Green Valley Farm",
  slug: "green-valley-farm"
});
// clerkOrg.id = "org_2qYYYYYYYYYY"

// 3. Add applicant as admin
await clerk.organizations.createOrganizationMembership({
  organizationId: "org_2qYYYYYYYYYY",
  userId: "user_2qXXXX",
  role: "org:admin"
});

// 4. Create tenant
INSERT INTO tenants (id, slug, farm_name) VALUES (
  'org_2qYYYYYYYYYY', 'green-valley-farm', 'Green Valley Farm'
);

// 5. Create subscription
INSERT INTO subscriptions (id, tenant_id, plan, status) VALUES (
  'sub_xxx', 'org_2qYYYYYYYYYY', 'free', 'active'
);

// 6. Create tenant_members
INSERT INTO tenant_members (tenant_id, user_id, role, status) VALUES (
  'org_2qYYYYYYYYYY', 'user_2qXXXX', 'farm_owner', 'active'
);

// 7. Update application
UPDATE farm_applications SET
  status = 'approved',
  assigned_farm_id = 'FARM-N669KHLF',
  assigned_tenant_id = 'org_2qYYYYYYYYYY',
  reviewed_at = NOW()
WHERE id = 'app_xxx';
```

#### Step 4: User Accesses Dashboard (December 5, 2025)

```javascript
// User visits /apply → Sees approval status → Clicks "Go to Dashboard"
// Redirects to /select-farm

// GET /api/user/farms returns:
{
  success: true,
  data: {
    hasFarms: true,
    farms: [{
      id: "org_2qYYYYYYYYYY",
      name: "Green Valley Farm",
      slug: "green-valley-farm",
      role: "farm_owner"
    }],
    applications: [{
      id: "app_xxx",
      farmName: "Green Valley Farm",
      status: "approved",
      assignedFarmId: "FARM-N669KHLF"
    }]
  }
}

// User clicks on farm → Clerk setActive() → Redirects to /dashboard
// Dashboard loads with farm context
```

---

## 8. Security & Best Practices

### Authentication

- ✅ All routes protected by Clerk middleware
- ✅ Organization context validated on tenant routes
- ✅ Super admin routes check platform_role

### Data Access

- ✅ Row-Level Security (RLS) on Supabase tables
- ✅ Tenant isolation via organization ID
- ✅ API routes validate orgId from Clerk session

### File Uploads

- ✅ Cloudinary for payment slips and images
- ✅ Signed URLs for secure access
- ✅ File type validation

### Error Handling

- ✅ Retry logic for Clerk operations
- ✅ Fallback mechanisms for failed operations
- ✅ Detailed error logging
- ✅ User-friendly error messages

### Best Practices

1. **Type Safety**: TypeScript + Drizzle ORM
2. **State Management**: React Query for server state
3. **Responsive Design**: Mobile-first with Tailwind
4. **Accessibility**: ARIA labels, keyboard navigation
5. **Performance**: Edge functions, caching
6. **Monitoring**: Error tracking, audit logs

---

## 9. AI Agent Rules & Memory

### Project Context

```yaml
project_name: MTK Dairy (Malik Tech Dairy)
type: Multi-Tenant SaaS Platform
domain: Dairy Farm Management
region: Pakistan
language: English + Urdu (RTL)
currency: PKR (Pakistan Rupees)
```

### Critical Rules for AI Agent

#### Database Rules

1. **Always use Supabase REST API** with type assertions for TypeScript
2. **Never use Drizzle ORM directly** in API routes - use Supabase client
3. **Tenant isolation is mandatory** - always filter by tenant_id
4. **Use `as any` type assertions** when Supabase types conflict

```typescript
// ✅ CORRECT
const { data, error } = await (supabase.from('tenants').select('*').eq('id', tenantId) as any);

// ❌ WRONG - direct Drizzle in API routes
const result = await db.select().from(tenants);
```

#### Clerk Integration Rules

1. **Organization ID = Tenant ID** in Supabase
2. **Always retry Clerk operations** with slug variations
3. **Add user to org after creating** organization
4. **Handle fallback IDs** (tenant_xxx) by creating Clerk org

```typescript
// Clerk org creation with retry
const slugVariations = [
  baseSlug,
  `${baseSlug}-${Date.now().toString(36)}`,
  `${baseSlug}-${Math.random().toString(36).slice(2, 8)}`,
];

for (const slug of slugVariations) {
  try {
    const org = await clerk.organizations.createOrganization({
      name: farmName,
      slug: slug,
    });
    // Success - add member
    await clerk.organizations.createOrganizationMembership({
      organizationId: org.id,
      userId: applicantId,
      role: 'org:admin',
    });
    break;
  } catch (error) {
    // Retry with next slug
  }
}
```

#### API Route Rules

1. **Always check auth** at the start of every route
2. **Return consistent JSON structure**: `{ success, data, error, message }`
3. **Log errors to console** with context
4. **Handle both success and error cases**

```typescript
// Standard API response format
return NextResponse.json({
  success: true,
  message: 'Operation successful',
  data: { ... }
});

return NextResponse.json(
  { success: false, error: 'Error message', details: '...' },
  { status: 400 }
);
```

#### UI/UX Rules

1. **Mobile-first responsive design** with Tailwind
2. **Use shadcn/ui components** for consistency
3. **Show loading states** for async operations
4. **Display toast notifications** for user feedback
5. **Use Lucide icons** for iconography

```typescript
// Mobile + Desktop responsive
<div className="lg:hidden">
  {/* Mobile card view */}
</div>
<div className="hidden lg:block">
  {/* Desktop table view */}
</div>
```

#### File Structure Rules

```
src/
├── app/
│   ├── (auth)/          # Clerk auth pages
│   ├── (dashboard)/     # Tenant dashboard (requires orgId)
│   ├── (onboarding)/    # Onboarding & farm selection
│   ├── (public)/        # Public pages (apply, etc.)
│   ├── (super-admin)/   # Super admin dashboard
│   └── api/             # API routes
├── components/
│   ├── ui/              # shadcn/ui components
│   └── tenant/          # Tenant-specific components
├── db/
│   └── schema.ts        # Drizzle ORM schema
├── lib/
│   ├── supabase.ts      # Supabase client
│   └── utils.ts         # Utility functions
└── middleware.ts        # Clerk middleware
```

### Memory for Future Sessions

```yaml
key_decisions:
  - Clerk Organizations used for multi-tenancy
  - Supabase PostgreSQL as primary database
  - Cloudinary for file storage
  - Free tier: 5 animals, 1 user
  - PKR currency for Pakistan market

known_issues:
  - Some tenants created with fallback IDs (tenant_xxx) need migration
  - Firebase still used for some legacy routes (migration pending)
  - Supabase types conflict with TypeScript (use `as any`)

common_fixes:
  - 404 on /login → Create redirect page to /sign-in
  - "No organizations" → Call /api/user/join-org to create/join
  - TypeScript errors → Use `as any` type assertions
  - Search not working → Check null safety in filter function

super_admin_emails:
  - kaash0542@gmail.com

test_accounts:
  - email: kaash0542@gmail.com
    farm: Green Valley Farm
    farmId: FARM-N669KHLF
    role: farm_owner
```

---

## Quick Reference Card

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                        MTK DAIRY QUICK REFERENCE                             │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  USER FLOW:                                                                 │
│  /sign-up → /apply → [Wait for Approval] → /select-farm → /dashboard       │
│                                                                             │
│  ADMIN FLOW:                                                                │
│  /super-admin/applications → Review → Approve/Reject                        │
│                                                                             │
│  KEY PAGES:                                                                 │
│  /sign-up, /sign-in    Authentication (Clerk)                               │
│  /apply                 Farm application form                               │
│  /select-farm           Farm selection (post-approval)                      │
│  /dashboard             Farm dashboard (requires org)                       │
│  /super-admin           Admin dashboard                                     │
│                                                                             │
│  KEY APIs:                                                                  │
│  /api/user/farms        Get user's farms                                    │
│  /api/user/join-org     Join Clerk organization                             │
│  /api/farm-applications Farm application CRUD                               │
│  /api/admin/applications/{id}/review  Approve/reject                        │
│                                                                             │
│  DATABASE:                                                                  │
│  Supabase Project: gdditqkvzlpnklcoxspj (ap-south-1)                        │
│                                                                             │
│  STORAGE:                                                                   │
│  Cloudinary: dujpst04m                                                      │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```

---

**Document Version**: 1.0.0  
**Last Updated**: December 5, 2024  
**Author**: AI Agent (Cascade)  
**Project**: MTK Dairy - Malik Tech Dairy Farm SaaS
