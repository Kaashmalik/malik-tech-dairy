...
describe("Critical Flow 3: Staff Invitation → Accept Invite → Log Milk with Limited Permissions", () => {
  ...
  it("should complete staff invitation and permission testing", () => {
    ...
    cy.get('a[href="/dashboard/animals/new"], button').contains(/add|new|create/i).first().click({ timeout: 10000 });
    ...
    cy.get('button').contains(/invite|add|new/i).first().click({ timeout: 10000 });
    ...
    cy.get('body').then(($body) => {
      if ($body.find('button:contains("Delete"), button[aria-label*="delete" i]').length > 0) {
        cy.get('button:contains("Delete"), button[aria-label*="delete" i]').first().should("be.disabled");
      }
    });
  });
});
